...
class Exp(MyExp):
    ...
    def get_gen_data_loader(self, is_distributed):
        ...
        dataset = UnSupMOTDataset(
            data_dir=os.path.join(get_yolox_datadir(), "pseu01_mot20"),
    ...
    def get_pseu_data_loader(self, batch_size, is_distributed, no_aug=False):
        ...
        dataset = UnSupMOTDataset(
            ...
            name='',
            action='load',
            pseu_pkl=os.path.join('YOLOX_outputs', 'r50_su01_un01_coco_mot20',
                                  'pseu_labels_ckpt.pth.tar'),
            img_size=self.input_size,
            preproc=TrainTransform(
                rgb_means=(0.485, 0.456, 0.406),
                std=(0.229, 0.224, 0.225),
    ...
    def dump_pseu_labels(self, data, rank=0, world_size=1):
        ...
        if world_size == 1:
            save_checkpoint(data, False, os.path.join(
                'YOLOX_outputs', self.exp_name), 'pseu_labels')
        else:
            save_checkpoint(data, False, os.path.join('YOLOX_outputs',
                            self.exp_name), 'pseu_labels.rank%d' % rank)
    ...
    def collect_pseu_labels(self, world_size=1):
        ...
        if world_size > 1:
            ...
            for rank in range(world_size):
                filename = os.path.join(path, ('pseu_labels.rank%d' % rank) + "_ckpt.pth.tar")
